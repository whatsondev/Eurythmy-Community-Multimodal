...
# Script to deploy the Eurythmy Community using Google Cloud Build
...
FRONTEND_URL=$(gcloud run services describe eurythmy-frontend --region "$REGION" --format 'value(status.url)' 2>/dev/null || echo "URL not found")
