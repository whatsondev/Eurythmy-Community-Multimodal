...
class GCSService:
    ...
    def detect_media_type(self, file_path: str) -> MediaType:
        ...
        if ext in self.config.TEXT_EXTENSIONS:
            ...
            if mime_type:
                if 'text'  in mime_type: return MediaType.TEXT
                if 'image' in mime_type: return MediaType.IMAGE
                if 'video' in mime_type: return MediaType.VIDEO
                if 'audio' in mime_type: return MediaType.AUDIO
            return MediaType.TEXT  # Default fallback

    def upload_file(self, file_path: str, practitioner_id: Optional[str] = None) -> Tuple[str, MediaType, str]:
        """Upload file to GCS, organised by media type"""
        if not os.path.exists(file_path):
            raise FileNotFoundError(f"File not found: {file_path}")

        media_type = self.detect_media_type(file_path)

        # Create organised path: media/{type}/{practitioner_id or 'unknown'}/{uuid}_{filename}
        practitioner_folder = practitioner_id or "unknown"
        # BUG FIX 1: blob_name used `survivor_folder` (undefined variable) instead of
        #            `practitioner_folder` which is defined on the line above.
        #            This caused a NameError runtime crash on every upload.
        blob_name = f"media/{media_type.value}/{practitioner_folder}/{uuid.uuid4()}_{os.path.basename(file_path)}"
