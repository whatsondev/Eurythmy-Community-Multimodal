...
def upload_media(file_path: str, practitioner_id: Optional[str] = None) -> Dict[str, Any]:
    """
    Upload media file to GCS and detect its type.
    
    Args:
        file_path: Path to the local file
        practitioner_id: Optional practitioner ID to associate with upload
    ...
    try:
        ...
        gcs_uri, media_type, signed_url = gcs_service.upload_file(file_path, practitioner_id)
...
def save_to_spanner(extraction_result: Any, practitioner_id: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        practitioner_id: Optional practitioner ID to associate with the broadcast
    ...
    try:
        ...
        stats = spanner_service.save_extraction_result(result_obj, practitioner_id)
...
async def process_media_upload(file_path: str, practitioner_id: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    """
    # Step 1: Upload
    upload_result = upload_media(file_path, practitioner_id)
    ...
    save_result = save_to_spanner(extraction_data['extraction_result'], practitioner_id)
