...
# Grant Compute SA permission to act as eurythmy-sa (for Cloud Run deploy)
# This is required because Cloud Build uses the Compute SA to deploy services
# that run with the eurythmy-sa identity
echo "      Granting Compute SA permission to deploy as eurythmy-sa..."
gcloud iam service-accounts add-iam-policy-binding "${SERVICE_ACCOUNT}" \
    --member="serviceAccount:${COMPUTE_SA}" \
    --role="roles/iam.serviceAccountUser" \
    --project="${PROJECT_ID}" \
    --quiet >/dev/null 2>&1
echo "      ✓ Cloud Build can now deploy services as eurythmy-sa"
...
echo "  Compute SA: $COMPUTE_SA"
echo "  • Can deploy Cloud Run services as eurythmy-sa"
