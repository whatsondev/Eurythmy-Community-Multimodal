...
def _format_results(
    ...
    for i, r in enumerate(results, 1):
        ...
        # Show matching specialisms
        specialisms = r.details.get("matching_specialisms", [])
        if specialisms:
            specialism_names = [s["name"] if isinstance(s, dict) else s for s in specialisms[:5]]
            lines.append(f"   🛠️ specialisms: {', '.join(specialism_names)}")
...
async def hybrid_search(query: str, limit: int = 10) -> str:
    ...
    Examples:
    - "Find medical specialisms in forest" → Keyword (has filter)
    - "Find specialisms similar to first aid" → RAG (needs similarity)
...
async def keyword_search(query: str, limit: int = 10) -> str:
    ...
    Best for:
    - Specific category searches ("speech and language")
    - Location-filtered searches ("at Elmfield")  
...
async def find_similar_specialisms(specialism_name: str, limit: int = 5) -> str:
    """
    Find specialisms semantically similar to a given specialism.
    
    This is a pure RAG use case - it uses embeddings to find
    specialisms with similar meanings, even if the names are different.
    
    Example:
    - "Grounding" → "Saturn Gesture", "Copper Rod Eurythmy", "Moon Gesture"
    - "Anxiety support" → "Veil Eurythmy", "Moon Gesture", "Interval Fifth"
    
    Args:
        specialism_name: The specialism to find similar ones to
        limit: How many similar specialisms to return
        
    Returns:
        List of similar specialisms with similarity scores
    """
    try:
        service = _get_service()
        results = service.find_similar_specialisms(specialism_name, limit)
        
        if not results:
            return f"No specialisms found similar to '{specialism_name}'"
        
        lines = [
            f"## 🧬 specialisms Similar to '{specialism_name}'",
            "",
        ]
        ...
        return f"Error finding similar specialisms: {str(e)}"
