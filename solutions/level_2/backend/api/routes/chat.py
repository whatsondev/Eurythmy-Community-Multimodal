...
async def chat(request: ChatRequest):
    try:
        ...
        if conversation_id in SESSION_MAP:
            ...
            try:
                await session_service.get_session(app_name="eurythmy -community", session_id=session_id, user_id=user_id)
            except Exception:
        ...
        if not session_id:
            ...
            session = await session_service.create_session(user_id=user_id, app_name="eurythmy -community")
